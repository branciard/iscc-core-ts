...
test('test_write_header', () => {
    ...
    expect(encode_header(0, 0, 0, 0)).toEqual(
        toHexString(bytes(0b00000000, 0b00000000))
    );
    expect(encode_header(1, 0, 0, 0)).toEqual(
        toHexString(bytes(0b00010000, 0b00000000))
    );
    expect(encode_header(7, 1, 1, 1)).toEqual(
        toHexString(bytes(0b01110001, 0b00010001))
    );
    expect(encode_header(8, 1, 1, 1)).toEqual(
        toHexString(bytes(0b10000000, 0b00010001, 0b00010000))
    );
    expect(encode_header(8, 8, 1, 1)).toEqual(
        toHexString(bytes(0b10000000, 0b10000000, 0b00010001))
    );
});
...
test('test_read_header', () => {
    ...
    // Test cases
    expect(decode_header(bytesToHex([0b00000000, 0b00000000]))).toEqual([
        0,
        0,
        0,
        0,
    ...
    expect(
        decode_header(bytesToHex([0b00000000, 0b00000000, 0b00000000]))
    ).toEqual([0, 0, 0, 0, new Uint8Array([0x00])]);

    expect(decode_header(bytesToHex([0b00010000, 0b00000000]))).toEqual([
        1,
        0,
        0,
        0,
    ...
    expect(decode_header(bytesToHex([0b01110001, 0b00010001]))).toEqual([
        7,
        1,
        1,
        1,
    ...
    expect(
        decode_header(bytesToHex([0b10000000, 0b00010001, 0b00010000]))
    ).toEqual([8, 1, 1, 1, new Uint8Array()]);

    expect(
        decode_header(bytesToHex([0b10000000, 0b10000000, 0b00010001]))
...
test('test_read_varnibble', () => {
    ...
    expect(decode_varnibble('1110111111111111101010')).toEqual([
        4679,
        '101010'
    ]);
});
...
test('test_decode_base32', () => {
    ...
    expect(decode_base32('MZXW6YQ')).toEqual(
        new Uint8Array([0x66, 0x6f, 0x6f, 0x62])
    ); // 'foob'
    expect(decode_base32('MZXW6YTB')).toEqual(
        new Uint8Array([0x66, 0x6f, 0x6f, 0x62, 0x61])
    ); // 'fooba'
    expect(decode_base32('MZXW6YTBOI')).toEqual(
...
test('test_base32hex_roundtrip_lower', () => {
    const data =
        'cc01cd9d2b7d247a8333f7b0b7d2cda8056c3d15eef738c1962e9148624feac1c14f';
    const expected_b32hex =
        'PG0SR79BFKI7L0PJUUOBFKMDL02MOF8LTRRJHGCM5Q8KGOIFTB0S2JO';
...
test('test_explain_maintype_meta', () => {
    expect(iscc_explain('AAAQCAAAAABAAAAA')).toBe(
        'META-NONE-V0-64-0100000002000000'
    );
});

test('test_explain_maintype_iscc_code', () => {
    const iscc = 'KICQOCPJM46YUUCBMWS6FFXRGM3LJOU5MZOVPOUHIJOHPI324GKN67Q';
    expect(iscc_explain(iscc)).toBe(
...
test('iscc_validate_mscdi', () => {
    const sample =
        'ISCC:KEDRRHYRYJ7XELW7HAO5FFGQRX75HJUKSUSZVWTTRNHTF2YL5SKP7XIUFXM4KMKXEZZA';
