...
describe('code-data', () => {
    ...
    test('softHashDataV0 handles empty input', async () => {
        ...
        expect(digest.toString('hex')).toBe('25f0bab671f506e1c532f892d9d7917a252e7a520832f5963a8cd4e9a7e312b5');
    });

    test('softHashDataV0 handles zero byte input', async () => {
        const digest = await soft_hash_data_v0(Buffer.from([0x00]));
        expect(digest.toString('hex')).toBe('770f8fd225ec1e5abb95e406afaddef303defe2f0d03b74c388f7b42ef96c7af');
    });
