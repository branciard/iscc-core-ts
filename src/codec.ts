...
import { CANONICAL_REGEX, MainTypes, MC_PREFIX_BUFFER, MULTIBASE, PREFIXES, ST, ST_ISCC, SUBTYPE_MAP, SubTypes, UNITS, Version } from './constants';
...
export function encode_header(
    ...
    return toHexString(encode_header_to_uint8Array(mtype, stype, version, length));
}
 ...
 */
export function decode_header(data: string): [MainTypes, SubTypes, Version, number, Uint8Array] {
    // Validate input
    ...
    if (remainingBits.length % 8 !== 0 && remainingBits.slice(0, 4) === '0000') {
...
export function iscc_clean(iscc: string): string {
    const split = iscc.trim().split(':').map(part => part.trim());
...
export function iscc_validate(iscc: string, { strict = true } = {}): boolean {
    ...
    if (expectedNBytes !== actualNBytes) {
        if (strict) {
            throw new Error(`Header expects ${expectedNBytes} but got ${actualNBytes} bytes`);
        }
        return false;
    }
...
export function iscc_normalize(isccCode: string): string {
    ...
    if (Object.keys(decoders).includes(multibasePrefix)) {
        ...
        if (!Buffer.from(decoded.slice(0, 2)).equals(MC_PREFIX_BUFFER)) {
            throw new Error(`Malformed multiformat codec: ${decoded.slice(0, 2)}`);
        }
        isccCode = encode_base32(Buffer.from(decoded.slice(2)).toString('hex'));
    } else {
        const prefix = isccCode.toUpperCase().replace("ISCC:", "").slice(0, 2) as (typeof PREFIXES)[number];
...
export function iscc_decompose(isccCode: string): string[] {
    ...
    while (rawCode.length > 0) {
        const [mt, st, vs, ln, body] = decode_header(Buffer.from(rawCode).toString('hex'));
        ...
        if (mt !== MainTypes.ISCC) {
            ...
            const code = encode_component(mt, st, vs, lnBits, Buffer.from(body.slice(0, bytesCount)).toString('hex'));
