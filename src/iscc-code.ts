...
export function gen_iscc_code_v0(codes: string[]): { iscc: string } {
    ...
    if (codes.length < 2) {
        throw new Error("Minimum two ISCC units required to generate valid ISCC-CODE");
    }
    for (const code of codes) {
        if (code.length < 16) {
            throw new Error(`Cannot build ISCC-CODE from units shorter than 64-bits: ${code}`);
        }
    }
    // Decode units and sort by MainType
    const decoded = codes
        .map(code => decode_header(Buffer.from(decode_base32(code)).toString('hex')))
        .sort((a, b) => a[0] - b[0]);
    ...
    if (new Set(sub_types).size > 1) {
        throw new Error("Semantic-Code and Content-Code must be of same SubType");
    }

    const st = sub_types.length > 0 
