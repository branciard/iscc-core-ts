...
test('test_hash_text_a', () => {
    ...
    expect(a).toBe("5f869a775c18bfbc3a117ab0114e13b2bf92614cda91513ee1f889fef3d6985f");
});

test('test_hash_text_b', () => {
    const a = Buffer.from(soft_hash_text_v0(TEXT_B)).toString('hex');
    expect(a).toBe("5f869a775c18bdfc3a117ab0114e13f2bf92610cda91513ee1f889bef3d6985f");
});

test('test_hash_text_c', () => {
    const a = Buffer.from(soft_hash_text_v0(TEXT_C)).toString('hex');
    expect(a).toBe("377b2f7b099a6df6bbc4a2ee4ff957b944c6434fa0e78842e7aad1169b71dd07");
});
...
test('test_code_text_b_128_bits', () => {
    ...
    expect(b).toEqual({ iscc: "ISCC:EABZHFKU6PNIXUVWYEEIQLOYHILX6", characters: 247 });
});

test('test_code_text_c_256_bits', () => {
    const c = gen_text_code(TEXT_C,256);
    expect(c).toEqual({ iscc: "ISCC:EADQE77SQ5NHKYPCDXT3E2NTB2EGV7VSKEUJDNXG2MICLCFZOPSDI4I", characters: 108 });
});

test('test_normalize_text', () => {
    const txt = "  Iñtërnâtiôn\nàlizætiøn☃💩 –  is a tric\t ky \u00A0 thing!\r";
...
test('test_code_text_empty', () => {
    ...
    expect(empty).toEqual({ iscc: "ISCC:EABSL4F2WZY7KBXBYUZPREWZ26IXU", characters: 0 });
});
